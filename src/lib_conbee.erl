...
device(Type,WantedName)->
    Result=case [{Name,NumId,ModelId,State,Config}||{Name,NumId,ModelId,State,Config}<-all_info(Type),
					     WantedName=:=Name] of
	       []->
...
format_info([{IdBin,Map}|T],Acc)->
    ...
    Config=not_defined,
    NewAcc=[{Name,NumId,ModelId,State,Config}|Acc],
